...
*/
    ...
    static void SnapAllAxes()
    {
        ...
        for (int i = 0; i < tranforms.Length; i++)
        {
            ...
            float x = EditorSnapSettings.move.x * Mathf.Round(pos.x / EditorSnapSettings.move.x);
            float y = EditorSnapSettings.move.y * Mathf.Round(pos.y / EditorSnapSettings.move.y);
            float z = EditorSnapSettings.move.z * Mathf.Round(pos.z / EditorSnapSettings.move.z);
#else
    ...
    [MenuItem("Window/NOT_Lonely/Object Placement Tool/Quick Actions/Set as First in Hierarchy %#_,", false, 10)]
    static void SetAsFirst()
    {
        {
            Transform[] tranforms = Selection.transforms;
            ...
            for (int i = 0; i < tranforms.Length; i++)
            {
                tranforms[i].SetAsFirstSibling();
            }
           
            Undo.RegisterCompleteObjectUndo(tranforms, "Set as first in Hierarchy");
        }
    }

    [MenuItem("Window/NOT_Lonely/Object Placement Tool/Quick Actions/Set as Last in Hierarchy %#_.", false, 10)]
    static void SetAsLast()
    {
        {
            Transform[] tranforms = Selection.transforms;
            ...
            for (int i = 0; i < tranforms.Length; i++)
            {
                tranforms[i].SetAsLastSibling();
            }

            Undo.RegisterCompleteObjectUndo(tranforms, "Set as last in Hierarchy");
        }
    }
