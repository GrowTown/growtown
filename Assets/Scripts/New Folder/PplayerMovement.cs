...
public class PplayerMovement : MonoBehaviour
{
    ...
    [Header("Gathering")]
    [SerializeField] private PlayerResourceInteractor resourceInteractor;
    ...
    void Start()
    {
        ...
        animator = GetComponent<Animator>();
        if (resourceInteractor == null)
            resourceInteractor = GetComponent<PlayerResourceInteractor>();
        CacheAnimationStateHash();
        ResetFishingAnimatorState(forceIdle: true, resetCastTrigger: true);
    ...
    void Update()
    {
        ...
        if (Input.GetKeyDown(KeyCode.E) && resourceInteractor != null)
            resourceInteractor.TriggerHit();
