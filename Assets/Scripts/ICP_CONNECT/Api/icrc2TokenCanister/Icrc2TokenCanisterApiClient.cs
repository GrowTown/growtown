...
using InternetClients.icrc2TokenCanister;
...
namespace InternetClients.icrc2TokenCanister
{
	public class Icrc2TokenCanisterApiClient
	{
		public IAgent Agent { get; }
		public Principal CanisterId { get; }
		public CandidConverter? Converter { get; }

		public Icrc2TokenCanisterApiClient(IAgent agent, Principal canisterId, CandidConverter? converter = default)
		{
			this.Agent = agent;
		...
		public async Task<Icrc2TokenCanisterApiClient.SymbolReturnArg0> Symbol()
		{
			CandidArg arg = CandidArg.FromCandid();
			QueryResponse response = await this.Agent.QueryAsync(this.CanisterId, "symbol", arg);
			CandidArg reply = response.ThrowOrGetReply();
			return reply.ToObjects<Icrc2TokenCanisterApiClient.SymbolReturnArg0>(this.Converter);
		}

		public async Task<Icrc2TokenCanisterApiClient.NameReturnArg0> Name()
		{
			CandidArg arg = CandidArg.FromCandid();
			QueryResponse response = await this.Agent.QueryAsync(this.CanisterId, "name", arg);
			CandidArg reply = response.ThrowOrGetReply();
			return reply.ToObjects<Icrc2TokenCanisterApiClient.NameReturnArg0>(this.Converter);
		}

		public async Task<Icrc2TokenCanisterApiClient.DecimalsReturnArg0> Decimals()
		{
			CandidArg arg = CandidArg.FromCandid();
			QueryResponse response = await this.Agent.QueryAsync(this.CanisterId, "decimals", arg);
			CandidArg reply = response.ThrowOrGetReply();
			return reply.ToObjects<Icrc2TokenCanisterApiClient.DecimalsReturnArg0>(this.Converter);
		}
		...
		public async Task<List<Icrc2TokenCanisterApiClient.Icrc1SupportedStandardsReturnArg0Item>> Icrc1SupportedStandards()
		{
			CandidArg arg = CandidArg.FromCandid();
			QueryResponse response = await this.Agent.QueryAsync(this.CanisterId, "icrc1_supported_standards", arg);
			CandidArg reply = response.ThrowOrGetReply();
			return reply.ToObjects<List<Icrc2TokenCanisterApiClient.Icrc1SupportedStandardsReturnArg0Item>>(this.Converter);
		}
